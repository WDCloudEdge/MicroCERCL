...
class AggrHGraphConvWindow(nn.Module):
    def __init__(self, hidden_size, output_size, graph: HeteroWithGraphIndex, num_heads: int = 2,
    ...
    def forward(self):
        ...
        for anomaly in self.graph.anomaly_index:
            graph_anomaly_index(graphs_anomaly_node_index, anomaly, self.graph.anomaly_index[anomaly], node_num,
                                instance_num)
        # return self.rnn_layer(th.stack(input_data_list, dim=1).T)[
        #     0], graphs_center_node_index, graphs_anomaly_node_index
        return th.stack(input_data_list, dim=1).T, graphs_center_node_index, graphs_anomaly_node_index
...
class AggrUnsupervisedGNN(nn.Module):
    ...
    def forward(self):
        aggr_feat, aggr_center_index, aggr_anomaly_index = self.conv()
        return aggr_feat, aggr_center_index, aggr_anomaly_index
    ...
    def loss(self, aggr_feat, aggr_center_index, aggr_anomaly_index):
        ...
        for center in aggr_index_combine:
            ...
            sum_criterion += self.criterion(aggr_center, torch.full_like(aggr_center, aggr_mean))
